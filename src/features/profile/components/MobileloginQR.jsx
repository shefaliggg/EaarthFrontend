...
function MobileloginQR() {
    ...
    const interval = setInterval(() => {
      ...
      if (diff === 0) {
        toast.error("QR Code Expired", {
          description:
            "The QR code is no longer valid. Please refresh to generate a new one.",
        });
  ...
  return (
    <motion.div
      ...
      className="rounded-3xl border shadow-md p-6 bg-card border-border"
    >
      <div className="flex items-center gap-6 p-4">
        {/* Right: Info */}
        <div className="flex-1 space-y-6">
          <h4 className="font-bold text-2xl mb-4 text-foreground ">
            Scan to Login & Access <br /> Your Account in Mobile
          </h4>
          ...
          <ul className="space-y-2 text-foreground">
            <li className="flex items-start gap-2">
              ...
              <span>
                <strong>Quick Login</strong> - Access your profile on mobile
                devices
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-primary">●</span>
              <span>
                <strong>Digital Crew Pass</strong> - Show your credentials on
                set
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-primary">●</span>
              <span>
                <strong>Contact Card</strong> - Share your professional info
                instantly
              </span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-primary">●</span>
              <span>
                <strong>Schedule Sync</strong> - View your call times and
                bookings
              </span>
            </li>
          </ul>
        ...
        {/* Center: QR Code */}
        <div className="bg-white p-4 rounded-lg border-2 border-primary relative">
          {isExpired && (
          ...
          {loading && (
            <div className="flex flex-col p-8 items-center text-center justify-center gap-4 text-purple-900 text-sm font-medium animate-pulse">
              <QrCode className="size-20 " /> Generating QR Code
            </div>
          )}
          {error && (
            <div className="flex flex-col p-8 items-center text-center justify-center gap-4 text-red-600 text-sm font-medium">
