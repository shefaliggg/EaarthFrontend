...
import { Eye, ZoomIn, ZoomOut, Download, X, Shield, CheckCircle, Calendar, User, Fingerprint, Mail, MapPin, Clock } from "lucide-react";

function SignatureCertificateModal({ profile, signatureImage, open, onOpenChange }) {
      ...
      <DialogContent className="w-screen max-w-screen! h-screen p-0 bg-transparent border-0 shadow-none overflow-hidden rounded-none">
        <div className="relative w-full h-full flex items-center justify-center bg-black/60 backdrop-blur-sm">
          ...
          <div className="absolute top-6 left-6 flex items-center gap-2 z-50 bg-black/60 backdrop-blur-xl rounded-full px-2 py-1.5 border border-white/10 shadow-lg">
            ...
            <span className="text-xs text-white/80 min-w-[48px] text-center font-mono">{zoomPercentage}%</span>
            <button
          ...
          >
            <div
              ...
              className={`transition-transform duration-200 ease-out py-12 ${zoom > 1 ? "cursor-grab active:cursor-grabbing" : ""
                }`}
              style={{
              ...
              <div className="w-[800px] h-[1131px] bg-white relative shadow-2xl rounded-2xl overflow-hidden flex flex-col">
                ...
                <div className="flex-1 p-10 relative">
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-400 font-mono border-b border-gray-100 pb-2 mb-5">
                    <span>Certificate ID: <span className="font-semibold text-gray-600">{certificateId}</span></span>
                  ...
                  {/* Signature Holder Section */}
                  <div className="mb-8">
                    <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Signature Owner</p>
                    <div className="bg-gray-50/80 rounded-xl p-4 border border-gray-100">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h2 className="text-lg font-bold text-gray-900">{profile?.name || "NICHOLAS PERRY"}</h2>
                  ...
                  <div className="mb-8">
                    <p className="text-xs text-gray-400 uppercase tracking-wider mb-2">Registered Signature</p>
                    <div className="border-2 border-dashed border-primary/30 rounded-xl p-6 bg-gray-50/50 flex items-center justify-center min-h-[180px]">
                      {signatureImage ? (
                        ...
                        <div className="text-center">
                          ...
                          <span className="text-gray-400 text-sm">Signature Preview</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    <p className="text-xs text-gray-400 uppercase tracking-wider mb-3">Certificate Timeline</p>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="bg-gray-50/50 rounded-lg p-3 text-center border border-gray-100">
                        <Calendar className="w-4 h-4 text-primary mx-auto mb-1" />
                        <p className="text-[11px] text-gray-400 uppercase">Signed Date</p>
                        <p className="text-sm font-semibold text-gray-800 mt-1">{signedDate}</p>
                      </div>
                      <div className="bg-gray-50/50 rounded-lg p-3 text-center border border-gray-100">
                        <CheckCircle className="w-4 h-4 text-green-500 mx-auto mb-1" />
                        <p className="text-[11px] text-gray-400 uppercase">Verified (Email OTP)</p>
                        <p className="text-sm font-semibold text-gray-800 mt-1">{verifiedDate}</p>
                      </div>
                      <div className="bg-gray-50/50 rounded-lg p-3 text-center border border-gray-100">
                        <Clock className="w-4 h-4 text-primary mx-auto mb-1" />
                        <p className="text-[11px] text-gray-400 uppercase">Activated Date</p>
                        <p className="text-sm font-semibold text-gray-800 mt-1">{activatedDate}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-semibold text-gray-900 mt-0.5">{profile?.id || "USR-" + certificateId.slice(-6)}</p>
                      </div>
                      <div>
                        <p className="text-gray-400 text-xs">Signature Type</p>
                        <p className="font-semibold text-gray-900 mt-0.5">Secure Digital Signature</p>
                      </div>
                      <div>
                        <p className="text-gray-400 text-xs">Verification Method</p>
                        <p className="font-semibold text-gray-900 mt-0.5">Email OTP Authenticated</p>
                      </div>
                      <div>
                  ...
                  <div className="mt-10 pt-6">
                    <div className="border-t border-gray-100 pt-4">
                      <p className="text-[10px] text-gray-400 text-center leading-relaxed">
                        This certificate is electronically generated and serves as official proof of digital signature registration.
                        The signature has been verified through email OTP authentication and is legally binding.
                      </p>
                      <div className="flex items-center justify-between mt-4">
                        <div>
                          <p className="text-[10px] text-gray-400">Authorized By</p>
                          <div className="w-32 h-px bg-gray-300 mt-1" />
                          <p className="text-xs font-medium text-gray-700 mt-1">System Authority</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] text-gray-400">Issued By</p>
                          <p className="text-sm font-semibold text-primary">Eaarth</p>
                        </div>
                      </div>
                    </div>
                  </div>
