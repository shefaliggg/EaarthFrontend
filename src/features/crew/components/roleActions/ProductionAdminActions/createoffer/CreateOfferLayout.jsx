...
export function CreateOfferLayout({
  ...
  // Returns only the single main contract form — belt-and-braces .slice(0,1)
  // on top of bundleMatcher already having isDefault:false on everything else.
  const getMainForm = () => {
    try {
      const bundle = getMatchedBundle(data);
      return bundle.forms.filter((f) => f.isDefault).slice(0, 1);
    } catch {
      return [];
    }
  };
  ...
  return (
    <div className="bg-purple-50/30 uppercase">
      ...
      <div className="px-3 py-2.5 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {data.otherDealProvisions && (
                  <div className="bg-amber-50/80 rounded-lg border border-amber-200/60 px-2 py-1">
                    ...
                    <HighlightField
                      fieldName="otherDealProvisions"
                      active={activeField === "otherDealProvisions"}
                ...
                {data.additionalNotes && (
                  <div className="bg-purple-50/60 rounded-lg border border-purple-200/60 px-2 py-1">
                    ...
                    <HighlightField
                      ...
                      active={activeField === "additionalNotes"}
                      onClick={onFieldClick}
                    >
        ...
        {/* ── Single main contract document only ── */}
        {!hideContractDocument && (() => {
          const mainForms = getMainForm();
          if (mainForms.length === 0) return null;
          return (
            <div className="space-y-2">
              {!hideOfferSections && (
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className="text-[9px] text-neutral-400 font-medium">A4 · Contract Preview</span>
                </div>
              )}
              <ContractDocument
                data={data}
