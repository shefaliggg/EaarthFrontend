...
function ChatBox() {
  ...
  return (
    <div className="rounded-3xl border bg-card shadow-sm h-[calc(100vh-38px)] max-h-[924px] flex flex-col mx-auto">
      ...
      <div className="border-t p-3 space-y-2.5 rounded-b-3xl flex-shrink-0 relative">
        {replyTo && (
            ...
            {attachments.map((att, index) => (
              <div key={index} className="relative w-20 h-20">
                {att.previewUrl ? (
                  ...
                  <div className="w-full h-full bg-primary/20 flex items-center justify-center text-[10px] rounded-lg">
                    <p className="w-full h-full overflow-hidden text-ellipsis p-1">{att.file.name}</p>
                  </div>
                )}
                <button
                  onClick={() =>
