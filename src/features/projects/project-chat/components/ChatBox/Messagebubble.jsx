...
export default function MessageBubble({
  ...
  const canEditMessage = (msg) => {
    ...
    return Date.now() - msg.timestamp < fifteenMinutes;
  };
      ...
      >
        ...
        >
          ...
          <AutoHeight className="w-full">
            ...
            >
              <div className="p-2 py-1">
                {/* Forwarded indicator */}
                {isForwarded && (
                  <div className={cn("flex items-center gap-1.5 mb-2 pb-2 border-b", isOwn ? "border-primary-foreground/20" : "border-border")}>
                    <CornerDownRight className={cn("w-3 h-3", isOwn ? "text-primary-foreground/70" : "text-muted-foreground")} />
                    <span className={cn("text-[10px] italic font-medium", isOwn ? "text-primary-foreground/70" : "text-muted-foreground")}>
                      Forwarded
                    </span>
                  </div>
                )}
                ...
                {message.replyTo && (
                  <div
                    ...
                    className={cn(
                      ...
                      isOwn ? "bg-primary/20 border-primary-foreground/50 hover:bg-primary/30" : "bg-background/60 border-primary hover:bg-background/80"
                    )}
                  >
                    <div className="text-[10px] font-semibold text-primary mb-0.5 truncate">
                      {message.replyTo.sender || "Unknown"}
                    </div>
                    <div className={cn("text-[11px] truncate", isOwn ? "text-primary-foreground/80" : "text-muted-foreground")}>
                ...
                {message.type === "video" && message.url && (
                  <video ref={videoRef} src={getFileUrl(message.url)} controls className="rounded-lg max-w-[300px] max-h-[300px]">
                  ...
                  <div className="flex items-center gap-3 min-w-[200px] bg-muted/50 p-3 rounded-lg">
                    ...
                    <div className="flex-1">
                      <div className="h-1 bg-muted rounded-full overflow-hidden">
                        <div className="h-full bg-primary transition-all" style={{ width: `${playProgress}%` }} />
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">Audio message</p>
                    </div>
                    <Volume2 className="w-4 h-4 text-muted-foreground" />
                    <audio ref={audioRef} src={getFileUrl(message.url)} onEnded={() => setIsPlaying(false)} />
                  </div>
                )}
                ...
                {(message.type === "file" || message.type === "document") && message.url && (
                  <div className="flex items-center gap-3 min-w-[200px]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {message.fileSize && <p className="text-xs text-muted-foreground">{message.fileSize}</p>}
                    </div>
                    <a href={getFileUrl(message.url)} target="_blank" rel="noopener noreferrer" download className="p-2 hover:bg-muted rounded-lg transition-colors">
                ...
                {isOwn && (
                  <div className="flex items-center justify-end gap-1 mt-1">
                    {message.edited && (
                      <span className={cn("text-[10px] italic mr-1", isOwn ? "text-primary-foreground/50" : "text-muted-foreground")}>
                        (edited)
                      </span>
                    )}
                    <span className="text-[10px] text-primary-foreground/70">{message.time}</span>
            ...
            {message.reactions && Object.keys(message.reactions).length > 0 && (
              <div className={cn("flex gap-1 mt-1", isOwn ? "flex-row-reverse" : "flex-row")}>
                {Object.entries(message.reactions).map(([emoji, count]) => (
                  <button key={emoji} className="bg-primary/20 hover:bg-muted px-2 py-1 rounded-full text-xs flex items-center gap-1 transition-all hover:scale-110">
                    <span>{emoji}</span>
            ...
            {/* Action buttons */}
            {showActions && (
              <div className={cn("flex gap-1 mt-1.5 transition-all duration-300 ease-out", isOwn ? "justify-end" : "justify-start")}>
                <ActionButton icon={Reply} tooltip="Reply" onClick={handleReply} />
                <ActionButton icon={Star} tooltip={isFavorited ? "Unstar" : "Star"} className={isFavorited ? "text-yellow-500" : ""} />
                <ActionButton icon={Forward} tooltip="Forward" onClick={() => setForwardDialogOpen(true)} />
                <ActionButton icon={Copy} tooltip="Copy" onClick={() => message.content && navigator.clipboard.writeText(message.content)} />
                <ActionButton icon={Smile} tooltip="React" onClick={() => setShowReactionPicker(showReactionPicker === message.id ? null : message.id)} />
                {isOwn && canEditMessage(message) && (
                  <ActionButton icon={Edit3} tooltip="Edit" className="text-blue-500" onClick={handleEdit} />
                )}
                <ActionButton icon={Trash2} tooltip="Delete" className="text-red-500" onClick={() => setDeleteDialogOpen(true)} />
              </div>
            )}
          </AutoHeight>
          ...
          {showReactionPicker === message.id && (
            <div className={cn("flex gap-1.5 mt-2 p-2 bg-card border rounded-xl shadow-lg z-10", isOwn ? "flex-row-reverse" : "flex-row")}>
              {REACTIONS.map((emoji) => (
                <button key={emoji} className="text-xl hover:scale-125 transition-transform p-1">
                  {emoji}
                </button>
              ))}
            </div>
...
function ActionButton({ icon: Icon, tooltip, className, onClick }) {
  return (
    <button
      title={tooltip}
      onClick={onClick}
      className={cn("p-1.5 rounded-lg bg-muted/80 hover:bg-muted transition-all duration-200 hover:scale-110 active:scale-95", className)}
      aria-label={tooltip}
    >
