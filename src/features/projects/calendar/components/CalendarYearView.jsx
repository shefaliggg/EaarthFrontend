...
export default function CalendarYearView({ currentDate, setCurrentDate, onDayClick, events }) {
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-primary/20 shadow-lg bg-card">
      {/* YEAR HEADER */}
      <div className="text-center py-4 border-b border-primary/20 bg-purple-50/80 dark:bg-purple-900/20">
        <h2 className="text-2xl font-black text-purple-800 dark:text-purple-300">
      ...
      <div className="grid grid-cols-3 gap-4 p-6">
        {months.map((monthDate) => {
          ...
          const monthEvents = events.filter((event) => {
            ...
            return eventMonth === monthDate.getMonth() && eventYear === monthDate.getFullYear();
          });

          return (
            <div
              key={monthDate}
              className="rounded-lg border border-primary/20 bg-card hover:shadow-md transition-all duration-200 overflow-hidden"
            >
              {/* MONTH HEADER */}
              <div className="bg-purple-50/80 dark:bg-purple-900/20 border-b border-primary/20 px-3 py-2 flex items-center justify-between">
              ...
              <div className="p-3">
                {/* WEEK LABELS */}
                <div className="grid grid-cols-7 text-[10px] font-black text-center text-muted-foreground mb-2">
                        ...
                        <TooltipTrigger asChild>
                          <div
                            ...
                            className={cn(
                              ...
                              !inMonth && "opacity-30",
                              isToday(date) && "bg-purple-200 dark:bg-purple-800/40 ring-2 ring-purple-400/30 font-bold scale-105",
                              hasEvents(date) && !isToday(date) && "font-semibold"
                            )}
                          >
                            {/* Date Number */}
                            <span className={cn(
                              ...
                              <div className="absolute bottom-0.5 flex gap-0.5">
                                {hasMultipleEvents ? (
                                  ...
                                  <span className={cn(
                                    ...
                                    getEventDotColor(dayEvents[0].eventType)
                                  )} />
                                )}
                              </div>
                            )}
                            ...
                            <div className="flex flex-col gap-2 p-1">
                              ...
                              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                                {dayEvents.slice(0, 3).map((event) => (
                                  <div key={event.id || event._id} className="text-xs">
      ...
      <div className="border-t border-primary/20 bg-purple-50/80 dark:bg-purple-900/20 px-6 py-4">
        <div className="grid grid-cols-4 gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-muted-foreground uppercase">Prep Events</p>
          </div>
          <div>
            <p className="text-2xl font-black text-peach-600 dark:text-peach-400">
              {events.filter(e => e.eventType === "shoot").length}
            </p>
            <p className="text-xs font-semibold text-muted-foreground uppercase">Shoot Events</p>
          </div>
          <div>
            <p className="text-2xl font-black text-mint-600 dark:text-mint-400">
              {events.filter(e => e.eventType === "wrap").length}
            </p>
            <p className="text-xs font-semibold text-muted-foreground uppercase">Wrap Events</p>
          </div>
        </div>
      </div>
    </div>
