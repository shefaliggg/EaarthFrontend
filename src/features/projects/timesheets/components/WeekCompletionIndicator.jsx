...
export function WeekCompletionIndicator({ weekEntries, isDarkMode = false, compact = false }) {
  const theme = {
    bg: isDarkMode ? 'bg-zinc-900' : 'bg-white',
    card: isDarkMode ? 'bg-zinc-800' : 'bg-purple-50/50',
    border: isDarkMode ? 'border-zinc-700' : 'border-purple-200',
  ...
  if (compact) {
    return (
      <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${theme.border} ${theme.card}`}>
